/**
 * Generated by "@vuepress/internal-site-data"
 */
export const siteData = {
  "title": "CcSir",
  "description": "这个杀手不太冷",
  "base": "/",
  "headTags": [
    [
      "meta",
      {
        "name": "baidu_union_verify",
        "content": "a0036ccf0b657813fb236f80f376c429"
      }
    ],
    [
      "link",
      {
        "rel": "icon",
        "href": "/images/favicon.svg"
      }
    ],
    [
      "meta",
      {
        "name": "cc",
        "content": "Cc的主页"
      }
    ],
    [
      "meta",
      {
        "name": "author",
        "content": "Cc的主页"
      }
    ]
  ],
  "pages": [
    {
      "title": "Home",
      "frontmatter": {
        "home": true,
        "tagline": "我是一只小小小小鸟~",
        "heroText": "CcSir",
        "bgImageStyle": {
          "height": "100vh"
        },
        "heroTextStyle": {
          "color": "#fff"
        },
        "bgImage": "/images/bg-logo.svg",
        "isShowTitleInHome": false,
        "readingShow": "top"
      },
      "regularPath": "/",
      "relativePath": "README.md",
      "key": "v-33c9af46",
      "path": "/",
      "lastUpdated": "2021年9月7日星期二凌晨2点57分"
    },
    {
      "title": "关于作者",
      "frontmatter": {
        "title": "关于作者",
        "date": "2020-05-01T00:00:00.000Z",
        "keys": [
          "e10adc3949ba59abbe56e057f20f883e"
        ],
        "publish": false,
        "readingShow": "top"
      },
      "regularPath": "/views/About/author.html",
      "relativePath": "views/About/author.md",
      "key": "v-aa010bea",
      "path": "/views/About/author.html",
      "headers": [
        {
          "level": 2,
          "title": "介绍",
          "slug": "介绍"
        },
        {
          "level": 2,
          "title": "我的经历",
          "slug": "我的经历"
        },
        {
          "level": 2,
          "title": "目标",
          "slug": "目标"
        },
        {
          "level": 2,
          "title": "联系我",
          "slug": "联系我"
        }
      ],
      "excerpt": "",
      "lastUpdated": "2021年9月7日星期二凌晨2点57分"
    },
    {
      "title": "ES6-JS进阶之面向对象-class",
      "frontmatter": {
        "title": "ES6-JS进阶之面向对象-class",
        "date": "2020-11-10T15:16:00.000Z",
        "categories": [
          "JavaScript"
        ],
        "tags": [
          "JavaScript",
          "ES6"
        ],
        "readingShow": "top"
      },
      "regularPath": "/views/JavaScript/ES6/",
      "relativePath": "views/JavaScript/ES6/readme.md",
      "key": "v-798ea9a9",
      "path": "/views/JavaScript/ES6/",
      "headers": [
        {
          "level": 2,
          "title": "一、面向对象导读",
          "slug": "一、面向对象导读"
        },
        {
          "level": 2,
          "title": "二、ES6中的类和对象",
          "slug": "二、es6中的类和对象"
        },
        {
          "level": 3,
          "title": "2.1 class 类",
          "slug": "_2-1-class-类"
        },
        {
          "level": 3,
          "title": "2.2 创建类",
          "slug": "_2-2-创建类"
        },
        {
          "level": 3,
          "title": "2.3 constructor 构造函数",
          "slug": "_2-3-constructor-构造函数"
        },
        {
          "level": 3,
          "title": "2.4 创建第一个类和对象",
          "slug": "_2-4-创建第一个类和对象"
        },
        {
          "level": 3,
          "title": "2.5 类中添加方法",
          "slug": "_2-5-类中添加方法"
        },
        {
          "level": 3,
          "title": "2.6 注意事项",
          "slug": "_2-6-注意事项"
        },
        {
          "level": 2,
          "title": "三、继承",
          "slug": "三、继承"
        },
        {
          "level": 3,
          "title": "3.1 语法",
          "slug": "_3-1-语法"
        },
        {
          "level": 3,
          "title": "3.2 super(超级的) 关键字",
          "slug": "_3-2-super-超级的-关键字"
        },
        {
          "level": 2,
          "title": "四、Tab栏 案例",
          "slug": "四、tab栏-案例"
        },
        {
          "level": 3,
          "title": "获取：",
          "slug": "获取"
        },
        {
          "level": 3,
          "title": "4.1 点击Tab栏能实现切换 Tab栏和内容",
          "slug": "_4-1-点击tab栏能实现切换-tab栏和内容"
        },
        {
          "level": 3,
          "title": "4.2 点击右上角×号能删除当前栏目和内容",
          "slug": "_4-2-点击右上角×号能删除当前栏目和内容"
        },
        {
          "level": 3,
          "title": "4.3 点击加号能实现增加Tab栏效果",
          "slug": "_4-3-点击加号能实现增加tab栏效果"
        },
        {
          "level": 3,
          "title": "4.4 双击Tab栏，可以编辑Tab栏名称",
          "slug": "_4-4-双击tab栏-可以编辑tab栏名称"
        },
        {
          "level": 3,
          "title": "4.5 双击内容，可编辑内容",
          "slug": "_4-5-双击内容-可编辑内容"
        },
        {
          "level": 3,
          "title": "4.6 细节方面：",
          "slug": "_4-6-细节方面"
        }
      ],
      "excerpt": "<div class=\"custom-block tip\"><p class=\"title\"></p><p><strong>面向过程POP</strong>就是分析出解决问题所需要的步骤，然后用函数把这些步骤一步一步实现，使用的时候再一个一个的依次调用就可以了。\n<strong>面向对象OOP</strong>是把事务分解成为一个个对象，然后由对象之间分工与合作。</p>\n</div>",
      "lastUpdated": "2021年9月7日星期二凌晨2点57分"
    },
    {
      "title": "JAVA开发手册",
      "frontmatter": {
        "title": "JAVA开发手册",
        "date": "2020-05-02T00:00:00.000Z",
        "categories": [
          "JAVA"
        ],
        "tags": [
          "JAVA"
        ],
        "readingShow": "top"
      },
      "regularPath": "/views/DevelopmentManual/JavaDevelopmentManual/JavaDevelopmentManual.html",
      "relativePath": "views/DevelopmentManual/JavaDevelopmentManual/JavaDevelopmentManual.md",
      "key": "v-952bada4",
      "path": "/views/DevelopmentManual/JavaDevelopmentManual/JavaDevelopmentManual.html",
      "headers": [
        {
          "level": 2,
          "title": "一、 编程规约",
          "slug": "一、-编程规约"
        },
        {
          "level": 3,
          "title": "(一) 命名风格",
          "slug": "一-命名风格"
        },
        {
          "level": 3,
          "title": "(二) 常量定义",
          "slug": "二-常量定义"
        },
        {
          "level": 3,
          "title": "(三) 代码格式",
          "slug": "三-代码格式"
        },
        {
          "level": 3,
          "title": "(四) OOP规约",
          "slug": "四-oop规约"
        },
        {
          "level": 3,
          "title": "(五) 集合处理",
          "slug": "五-集合处理"
        },
        {
          "level": 3,
          "title": "(六) 并发处理",
          "slug": "六-并发处理"
        },
        {
          "level": 3,
          "title": "(七) 控制语句",
          "slug": "七-控制语句"
        },
        {
          "level": 3,
          "title": "(八) 注释规约",
          "slug": "八-注释规约"
        },
        {
          "level": 3,
          "title": "(九) 其它",
          "slug": "九-其它"
        },
        {
          "level": 2,
          "title": "二、异常日志",
          "slug": "二、异常日志"
        },
        {
          "level": 3,
          "title": "(一) 异常处理",
          "slug": "一-异常处理"
        },
        {
          "level": 3,
          "title": "(二) 日志规约",
          "slug": "二-日志规约"
        },
        {
          "level": 2,
          "title": "三、单元测试",
          "slug": "三、单元测试"
        },
        {
          "level": 2,
          "title": "四、安全规约",
          "slug": "四、安全规约"
        },
        {
          "level": 2,
          "title": "五、MySQL数据库",
          "slug": "五、mysql数据库"
        },
        {
          "level": 3,
          "title": "(一) 建表规约",
          "slug": "一-建表规约"
        },
        {
          "level": 3,
          "title": "(二) 索引规约",
          "slug": "二-索引规约"
        },
        {
          "level": 3,
          "title": "(三) SQL语句",
          "slug": "三-sql语句"
        },
        {
          "level": 3,
          "title": "(四) ORM映射",
          "slug": "四-orm映射"
        },
        {
          "level": 2,
          "title": "六、工程结构",
          "slug": "六、工程结构"
        },
        {
          "level": 3,
          "title": "(一) 应用分层",
          "slug": "一-应用分层"
        },
        {
          "level": 3,
          "title": "(二) 二方库依赖",
          "slug": "二-二方库依赖"
        },
        {
          "level": 3,
          "title": "(三) 服务器",
          "slug": "三-服务器"
        },
        {
          "level": 2,
          "title": "七、设计规约",
          "slug": "七、设计规约"
        },
        {
          "level": 2,
          "title": "附录：专有名词解释",
          "slug": "附录-专有名词解释"
        }
      ],
      "lastUpdated": "2021年9月7日星期二凌晨2点57分"
    },
    {
      "title": "MySQL",
      "frontmatter": {
        "readingShow": "top"
      },
      "regularPath": "/views/SQL/MySQL/MySQL.html",
      "relativePath": "views/SQL/MySQL/MySQL.md",
      "key": "v-6cdc1d4e",
      "path": "/views/SQL/MySQL/MySQL.html",
      "headers": [
        {
          "level": 2,
          "title": "MySQL",
          "slug": "mysql"
        }
      ],
      "lastUpdated": "2021年9月7日星期二凌晨2点57分"
    },
    {
      "title": "SQL Server",
      "frontmatter": {
        "readingShow": "top"
      },
      "regularPath": "/views/SQL/SQL_Server/SQL_Server.html",
      "relativePath": "views/SQL/SQL_Server/SQL_Server.md",
      "key": "v-59a194f0",
      "path": "/views/SQL/SQL_Server/SQL_Server.html",
      "headers": [
        {
          "level": 2,
          "title": "SQL Server",
          "slug": "sql-server"
        }
      ],
      "lastUpdated": "2021年9月7日星期二凌晨2点57分"
    },
    {
      "title": "Oracle",
      "frontmatter": {
        "readingShow": "top"
      },
      "regularPath": "/views/SQL/Oracle/Oracle.html",
      "relativePath": "views/SQL/Oracle/Oracle.md",
      "key": "v-7d73b008",
      "path": "/views/SQL/Oracle/Oracle.html",
      "headers": [
        {
          "level": 2,
          "title": "Oracle",
          "slug": "oracle"
        }
      ],
      "lastUpdated": "2021年9月7日星期二凌晨2点57分"
    },
    {
      "frontmatter": {
        "layout": "Tags",
        "title": "Tags",
        "readingShow": "top"
      },
      "regularPath": "/tag/",
      "key": "v-b1564aac",
      "path": "/tag/"
    },
    {
      "frontmatter": {
        "layout": "FrontmatterKey",
        "title": "Categories",
        "readingShow": "top"
      },
      "regularPath": "/categories/",
      "key": "v-ef9325c4",
      "path": "/categories/"
    },
    {
      "frontmatter": {
        "layout": "TimeLines",
        "title": "Timeline",
        "readingShow": "top"
      },
      "regularPath": "/timeline/",
      "key": "v-6319eb4e",
      "path": "/timeline/"
    },
    {
      "frontmatter": {
        "layout": "Tag",
        "title": "JavaScript Tags",
        "readingShow": "top"
      },
      "regularPath": "/tag/JavaScript/",
      "key": "v-7a5f6990",
      "path": "/tag/JavaScript/"
    },
    {
      "frontmatter": {
        "layout": "Tag",
        "title": "ES6 Tags",
        "readingShow": "top"
      },
      "regularPath": "/tag/ES6/",
      "key": "v-3257dcde",
      "path": "/tag/ES6/"
    },
    {
      "frontmatter": {
        "layout": "Tag",
        "title": "JAVA Tags",
        "readingShow": "top"
      },
      "regularPath": "/tag/JAVA/",
      "key": "v-18263c3a",
      "path": "/tag/JAVA/"
    },
    {
      "frontmatter": {
        "layout": "Category",
        "title": "JavaScript Categories",
        "readingShow": "top"
      },
      "regularPath": "/categories/JavaScript/",
      "key": "v-081e6844",
      "path": "/categories/JavaScript/"
    },
    {
      "frontmatter": {
        "layout": "Category",
        "title": "JAVA Categories",
        "readingShow": "top"
      },
      "regularPath": "/categories/JAVA/",
      "key": "v-6128b7ef",
      "path": "/categories/JAVA/"
    }
  ],
  "themeConfig": {
    "type": "blog",
    "fullscreen": true,
    "logo": "/images/favicon.svg",
    "blogConfig": {
      "category": {
        "location": 2,
        "text": "Category"
      },
      "tag": {
        "location": 3,
        "text": "Tag"
      }
    },
    "author": "CcSir",
    "authorAvatar": "/images/avatar.svg",
    "nav": [
      {
        "text": "主页",
        "link": "/",
        "icon": "reco-home"
      },
      {
        "text": "Java",
        "link": "/views/Java/",
        "items": [
          {
            "text": "JavaSE",
            "link": "/views/Java/JavaSE/JavaDevelopmentManual1"
          },
          {
            "text": "JavaEE",
            "link": "/views/Java/JavaEE/JavaDevelopmentManual2"
          }
        ]
      },
      {
        "text": "数据库",
        "link": "/views/SQL/",
        "items": [
          {
            "text": "数据库基础和原理",
            "items": [
              {
                "text": "数据库原理",
                "link": "/views/SQL/MySQL1/JavaDevelopmentManual1"
              },
              {
                "text": "SQL语言",
                "link": "/views/SQL/MySQL2/JavaDevelopmentManual1"
              }
            ]
          },
          {
            "text": "SQL 数据库",
            "items": [
              {
                "text": "MySQL",
                "link": "/views/SQL/MySQL/MySQL"
              },
              {
                "text": "Oracle",
                "link": "/views/SQL/Oracle/Oracle"
              },
              {
                "text": "SQL Server",
                "link": "/views/SQL/SQL_Server/SQL_Server"
              }
            ]
          },
          {
            "text": "NoSQL 数据库",
            "items": [
              {
                "text": "Redis",
                "link": "/views/SQL/Redis/JavaDevelopmentManual1"
              },
              {
                "text": "MongoDB",
                "link": "/views/SQL/MongoDB/JavaDevelopmentManual1"
              },
              {
                "text": "ElasticSearch",
                "link": "/views/SQL/ElasticSearch/JavaDevelopmentManual1"
              }
            ]
          }
        ]
      },
      {
        "text": "框架",
        "link": "/views/Framework/",
        "items": [
          {
            "text": "Spring",
            "link": "/views/Framework/JavaDevelopmentManual/JavaDevelopmentManual1"
          },
          {
            "text": "SpringMVC",
            "link": "/views/Framework/JavaDevelopmentManual/JavaDevelopmentManual2"
          },
          {
            "text": "SpringBoot",
            "link": "/views/Framework/JavaDevelopmentManual/JavaDevelopmentManual3"
          },
          {
            "text": "SpringCloud",
            "link": "/views/Framework/JavaDevelopmentManual/JavaDevelopmentManual4"
          }
        ]
      },
      {
        "text": "中间件",
        "link": "/views/Middleware/",
        "items": [
          {
            "text": "Kafka",
            "link": "/views/Middleware/JavaDevelopmentManual/JavaDevelopmentManual1"
          },
          {
            "text": "RocketMQ",
            "link": "/views/Middleware/JavaDevelopmentManual/JavaDevelopmentManual2"
          }
        ]
      },
      {
        "text": "数据结构与算法",
        "link": "/views/DataStructureAndAlgorithm/",
        "items": [
          {
            "text": "数据结构",
            "link": "/views/DataStructureAndAlgorithm/JavaDevelopmentManual/JavaDevelopmentManual1"
          },
          {
            "text": "算法",
            "link": "/views/DataStructureAndAlgorithm/JavaDevelopmentManual/JavaDevelopmentManual2"
          }
        ]
      },
      {
        "text": "架构",
        "link": "/views/Architect/",
        "items": [
          {
            "text": "架构基础",
            "link": "/views/Architect/JavaDevelopmentManual1"
          },
          {
            "text": "方法论",
            "link": "/views/Architect/JavaDevelopmentManual2"
          },
          {
            "text": "分布式系统",
            "link": "/views/Architect/JavaDevelopmentManual3"
          },
          {
            "text": "微服务系统与设计",
            "link": "/views/Architect/JavaDevelopmentManual4"
          },
          {
            "text": "系统设计和架构案例",
            "link": "/views/Architect/JavaDevelopmentManual5"
          }
        ]
      },
      {
        "text": "开发手册",
        "link": "/views/DevelopmentManual/",
        "items": [
          {
            "text": "JAVA开发手册",
            "link": "/views/DevelopmentManual/JavaDevelopmentManual/JavaDevelopmentManual"
          }
        ]
      },
      {
        "text": "工具",
        "link": "/views/Tool/",
        "items": [
          {
            "text": "Maven",
            "link": "/views/Tool/JavaDevelopmentManual1"
          },
          {
            "text": "Gradle",
            "link": "/views/Tool/JavaDevelopmentManual2"
          },
          {
            "text": "Eclipse",
            "link": "/views/Tool/JavaDevelopmentManual3"
          },
          {
            "text": "IntelliJ IDEA",
            "link": "/views/Tool/JavaDevelopmentManual4"
          }
        ]
      },
      {
        "text": "项目",
        "link": "/views/Project/",
        "items": [
          {
            "text": "A项目",
            "link": "/views/Project/JavaDevelopmentManual1"
          },
          {
            "text": "B项目",
            "link": "/views/Project/JavaDevelopmentManual2"
          }
        ]
      },
      {
        "text": "项目管理",
        "link": "/views/ProjectManagement/",
        "items": [
          {
            "text": "PMP",
            "link": "/views/ProjectManagement/JavaDevelopmentManual1"
          },
          {
            "text": "ACP",
            "link": "/views/ProjectManagement/JavaDevelopmentManual2"
          }
        ]
      },
      {
        "text": "关于",
        "icon": "reco-message",
        "items": [
          {
            "text": "关于我",
            "link": "/views/About/author",
            "icon": "reco-account"
          },
          {
            "text": "简历",
            "link": "/views/About/resume",
            "icon": "reco-document"
          },
          {
            "text": "CSDN",
            "link": "https://blog.csdn.net/hutuxiaogui",
            "icon": "reco-csdn"
          },
          {
            "text": "GitHub",
            "link": "https://github.com/hutuxiaogui",
            "icon": "reco-github"
          },
          {
            "text": "Gitee",
            "link": "https://gitee.com/hutuxiaogui",
            "icon": "reco-mayun"
          }
        ]
      }
    ],
    "sidebar": {
      "/views/DevelopmentManual/": [
        {
          "title": "JAVA开发手册",
          "collapsable": true,
          "children": [
            "/views/DevelopmentManual/JavaDevelopmentManual/JavaDevelopmentManual"
          ]
        }
      ],
      "/views/SQL/": [
        {
          "title": "数据库",
          "collapsable": true,
          "children": [
            "/views/SQL/MySQL/MySQL",
            "/views/SQL/Oracle/Oracle",
            "/views/SQL/SQL_Server/SQL_Server"
          ]
        }
      ]
    },
    "subSidebar": "auto",
    "mode": "dark",
    "modePicker": true,
    "smooth": "true",
    "noFoundPageByTencent": false
  }
}